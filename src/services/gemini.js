...
export async function breakdownWeeklyGoal(weeklyGoal, availableDays) {
  const systemPrompt = `You are a supportive planning assistant helping someone break down a goal into manageable tasks. The goal could be for work, study, health, a personal project, or any other area of life. Break the goal into smaller tasks distributed across the available days. Do NOT assign any clock times — only assign which day of the week each task belongs to, or leave a task unassigned if it's better done whenever there is time. Output ONLY valid JSON: [{"day": "Monday" or null, "task": "short description"}]. Keep each task under 10 words. Distribute realistically — not every day needs a task.`;
